...
### Option 1:
if option==1:
    # unwindowed, no signal
    use_mask = False
    include_synthetic_b = False
elif option==2:
    # windowed, with signal
    use_mask = True
...
# FISHER
fish_out = '/projects/QUIJOTE/Oliver/polybin_testing/bfish%d_%d.npy'%(option,fish_index)
if not os.path.exists(fish_out) and fish_index < N_it:
    init = time.time()
    this_fish = bspec.compute_fisher_contribution(fish_index, verb=True)
    print("Fisher matrix %d computed in %.2f s"%(fish_index,time.time()-init))

    np.save(fish_out,this_fish)

# IDEAL NUMERATOR
ideal_out = '/projects/QUIJOTE/Oliver/polybin_testing/bnum%d_ideal%d.npy'%(option,fish_index)
if not os.path.exists(ideal_out) and fish_index < N_sim:
    
    init = time.time()
    num_ideal = bspec.Bl_numerator_ideal(sim)
    print("Ideal numerator %d computed in %.2f s"%(fish_index,time.time()-init))
    
    np.save(ideal_out,num_ideal)

# OPTIMAL NUMERATOR
optimal_out = '/projects/QUIJOTE/Oliver/polybin_testing/bnum%d_opt%d.npy'%(option,fish_index)
if not os.path.exists(optimal_out) and fish_index < N_sim:

    # Generate debiasing simultions
    bspec.generate_sims(N_it,b_input=b_input_fac,add_B=include_synthetic_b,remove_mean=True,preload=True)

    init = time.time()
    num = bspec.Bl_numerator(sim, verb=True)
    print("Optimal numerator %d computed in %.2f s"%(fish_index,time.time()-init))
    
    np.save(optimal_out,num)

# OPTIMAL NUMERATOR (w/o 1-field subtraction)
optimal_out1 = '/projects/QUIJOTE/Oliver/polybin_testing/bnum%d_opt%d_no1.npy'%(option,fish_index)
if not os.path.exists(optimal_out1) and fish_index < N_sim:
    init = time.time()
    num1 = bspec.Bl_numerator(sim, include_linear_term=False, verb=True)
    print("Optimal numerator %d (w/o 1-field) computed in %.2f s"%(fish_index,time.time()-init))
    
    np.save(optimal_out1,num1)
