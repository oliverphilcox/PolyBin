...
pol=False
backend = 'libsharp'
outdir = '/mnt/ceph/users/ophilcox/polybin_testing/Bl_unpol/'
if not os.path.exists(outdir): os.makedirs(outdir)

# Bin edges (could also be non-linearly spaced)
l_bins = np.arange(2, 153, 10)
Nl = len(l_bins)-1
...
bspec = pb.BSpec(base, mask, applySinv, l_bins, fields=fields, parity='even')
...
if not os.path.exists(out_fish):
    if sim_no*10<N_it:
        ...
        for ii in range(r_fish*sim_no,r_fish*sim_no+r_fish):
            ...
            fishs.append(bspec.compute_fisher_contribution(ii, verb=(ii==r_fish*sim_no)))
        fishs = np.asarray(fishs)
        np.save(out_fish, fishs)
        print("Fisher computation complete, avering %.2f time, %d / %d SHTs"%((time.time()-t1)/r_fish,base.n_SHTs_forward//r_fish, base.n_SHTs_reverse//r_fish))
