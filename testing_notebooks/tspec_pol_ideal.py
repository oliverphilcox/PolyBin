...
# out_unwindowed4 = outdir+'Tl_unwindowed_4field%d_%d.npy'%(option,sim_no)
# out_unwindowed0 = outdir+'Tl_unwindowed_0field%d_%d.npy'%(option,sim_no)
# out_unwindowed = outdir+'Tl_unwindowed%d_%d.npy'%(option,sim_no)
...
#         # Precompute MC data
#         t2 = time.time()
#         tspec.generate_sims(N_it, preload=True, verb=True)
#         print("Generating MC data took %.2f seconds"%(time.time()-t2))
...
#             sim = mask*raw_sim    
#             tl_unwindowed.append(tspec.Tl_numerator(sim, verb=(ii==r_sim*sim_no), include_disconnected_term=True))
#             tl_unwindowed0.append(tspec.t0_num/tspec.sym_factor)
...
#             raw_sim = base.generate_data(seed=ii, add_B=False)
#             sim = mask*raw_sim
#             tl_unwindowed4.append(tspec.Tl_numerator(sim, verb=(ii==r_sim*sim_no), include_disconnected_term=False))
#         print("Unwindowed sim computation complete, averaging %.2f time, %d / %d SHTs"%((time.time()-t1)/r_sim,base.n_SHTs_forward//r_sim,base.n_SHTs_reverse//r_sim))

#         tl_unwindowed = np.asarray(tl_unwindowed)
#         tl_unwindowed0 = np.asarray(tl_unwindowed0)
#         tl_unwindowed4 = np.asarray(tl_unwindowed4)

#         np.save(out_unwindowed4,tl_unwindowed4)
#         np.save(out_unwindowed0,tl_unwindowed0)
#         np.save(out_unwindowed,tl_unwindowed)

# Zero cou/mnt/home/ophilcox/ceph/ (for diagnostics only)
base.n_SHTs_forward, base.n_SHTs_reverse = 0, 0

### Ideal simulations
out_ideal = outdir+'Tl_ideal%d_%d.npy'%(option,sim_no)
out0_ideal = outdir+'Tl0_ideal%d_%d.npy'%(option,sim_no)
if not os.path.exists(out_ideal):
    if sim_no*r_sim<N_sim: 
        ...
        for ii in range(r_sim*sim_no,r_sim*sim_no+r_sim):
            ...
            raw_sim = base.generate_data(seed=ii, add_B=False)
            sim = mask*raw_sim
            tl_ideal.append(tspec.Tl_numerator_ideal(sim, verb=True, N_cpus=4))
        tl_ideal = np.asarray(tl_ideal)
        ...
        np.save(out_ideal,tl_ideal)
        
        t0_num = tspec.t0_num_ideal
        np.save(out0_ideal, t0_num)
