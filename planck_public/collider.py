...
# NB: K_i = k_i / s
u_arr = np.arange(-0.99,1.00,0.2)[np.newaxis,np.newaxis,:] 
v_arr = np.arange(1.05,21.,0.2)[np.newaxis,:,np.newaxis]
K1_arr = (u_arr+v_arr)/2.
K2_arr = (v_arr-u_arr)/2.
dv = np.diff(v_arr.ravel())[0]
du = np.diff(u_arr.ravel())[0]

s_arr = np.logspace(-5,0,40)
...
def compute_f(L1):
    ...
    for L2 in range(lmax_sq+3):
        ...
        for Lp in range(abs(L1-L2),min(L1+L2+1,Lmax+2),2):
            ...
            if L1>100 or L2>100 or Lp>100:
                summ2 = np.inf*leg_DDelta[0]
            else:
