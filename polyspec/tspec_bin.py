...
class TSpecBin():
    """
    Binned trispectrum estimation class. This takes the binning strategy as input and a base class. 
    ...
    def __init__(self, base, mask, applySinv, l_bins, l_bins_squeeze=[], L_bins=[], include_partial_triangles=False, fields=['TTTT','TTTE','TTEE','TTBB','TETE','TEEE','TEBB','TBTB','TBEB','EEEE','EEBB','EBEB','BBBB','TTTB','TTEB','TETB','TEEB','TBEE','TBBB','EEEB','EBBB'], parity='both'):
        ...
        if self.include_partial_triangles:
            print("Including triangles partially satisfying the triangle conditions (in the lowest bin only)")
    ...
    def _check_bin(self, bin1, bin2, bin3):
        ...
        """
        if ((bin1<=0) or (bin2<=0) or (bin3<=0)) and self.include_partial_triangles:
            good = 0
            ...
            for u in self.fields:
                ...
                for bin1 in range(self.Nl_squeeze):
                    for bin2 in range(self.Nl_squeeze):
                        ...
                        if min([bin1,bin2])>=self.Nl: continue
                        for bin3 in range(self.Nl_squeeze):
                            ...
                            for bin4 in range(self.Nl_squeeze):
                                ...
                                if min([bin3,bin4])>=self.Nl: continue
    ...
    def _compute_H_maps(self, h_lm):
        ...
        H_pm1_maps = []
        for bin1 in range(self.Nl_squeeze):
            H_pm1_maps.append([np.asarray([[1],[-1]])*self.base.to_map_spin(self.ell_bins[bin1]*h_lm[i].conj(),-1.*self.ell_bins[bin1]*h_lm[i].conj(),1) for i in range(len(h_lm))])
        return H_pm1_maps
    ...
    def _process_sim(self, sim_pair, input_type='map'):
        ...
        """
        # Apply S^-1 and transform to Fourier-space
        h_alpha_lm = self.applySinv(sim_pair[0], input_type=input_type)
        h_beta_lm = self.applySinv(sim_pair[1], input_type=input_type)
        
        # Compute H_alpha and H_beta maps
        H_a_maps = self._compute_H_maps(h_alpha_lm)
        H_b_maps = self._compute_H_maps(h_beta_lm)
    ...
    def generate_sims(self, N_pairs, Cl_input=[], preload=True, verb=False):
        ...
        if preload:
            ...
            for ii in range(self.N_it):
                ...
                if self.ones_mask:
                    alpha_lm = self.base.generate_data(int(1e5)+ii, Cl_input=Cl_input, output_type='harmonic', deconvolve_beam=False)
                    beta_lm = self.base.generate_data(int(2e5)+ii, Cl_input=Cl_input, output_type='harmonic', deconvolve_beam=False)
    ...
    def Tl_numerator(self, data, include_disconnected_term=True, verb=False):
        ...
        H_maps = self._compute_H_maps(h_data_lm)
        ...
        if include_disconnected_term:
            ...
            for ii in range(self.N_it):
                if not compute_t0:
                    if verb: print("# Assembling 2-field trispectrum numerator for simulation pair %d of %d "%(ii+1,self.N_it))
                else:
                    if verb: print("# Assembling 2-field and 0-field trispectrum numerator for simulation pair %d of %d"%(ii+1,self.N_it))
    ...
    def compute_fisher_contribution(self, seed, verb=False):
        ...
        # Compute two random (beam-deconvolved) realizations with known power spectrum
        if verb: print("# Generating GRFs")
        a_maps = []
        for ii in range(2):
            a_maps.append(self.base.generate_data(seed=seed+int((1+ii)*1e9), deconvolve_beam=True, output_type='harmonic'))
                
        # Define Q map code
        def compute_Q4(weighting):
            """
            Assemble and return an array of Q4 maps in real- or harmonic-space, for S^-1.P or A^-1 weighting. 
            ...
            H_maps = [self._compute_H_maps(Uinv_a_lm) for Uinv_a_lm in Uinv_a_lms]
            ...
            tmp_Q111 = np.zeros((self.N_t,1+2*self.pol,len(Uinv_a_lms[0][0])),dtype='complex')
            tmp_Q222 = np.zeros((self.N_t,1+2*self.pol,len(Uinv_a_lms[0][0])),dtype='complex')
            tmp_Q112 = np.zeros((self.N_t,1+2*self.pol,len(Uinv_a_lms[0][0])),dtype='complex')
            tmp_Q122 = np.zeros((self.N_t,1+2*self.pol,len(Uinv_a_lms[0][0])),dtype='complex')
                                ...
                                def add_Q4_element(u3_index, u4_index, bin3_index, bin4_index, these_ind):
                                    ...
                                    for ii in these_ind:
                                        ...
                                        def add_to_tmp_Q(bin3, u3, bin4, u4, HA_lms3, HA_lms4, tmp_Q):
                                            ...
                                            this_Q = np.zeros((1+2*self.pol,2,len(Uinv_a_lms[0][0])),dtype='complex')

                                            ## Add all permutations (noting that we have already symmetrized over the two indices of A)
                                            this_Q[u3] -= 1./self.sym_factor[ii]*self.ell_bins[bin3]*HA_lms4.conj()  
                                            this_Q[u4] -= 1./self.sym_factor[ii]*self.ell_bins[bin4]*HA_lms3.conj()  
            ...
            def weight_Q_maps(tmp_Q):
                # Optionally apply S^-1.P weighting and add to output arrays
                Q_maps = np.zeros((self.N_t,len(np.asarray(self.beam_lm).ravel())),dtype='complex')
        ...
        fish -= 3.*(Q4_Sinv[0].conj())@(Q4_Ainv[3].T)
        fish -= 3.*(Q4_Sinv[1].conj())@(Q4_Ainv[2].T)        
        fish -= 3.*(Q4_Sinv[3].conj())@(Q4_Ainv[0].T)
        fish -= 3.*(Q4_Sinv[2].conj())@(Q4_Ainv[1].T)        
    ...
    def _compute_t0_t2_ideal(self, Cinv_data_lm, N_cpus=4, verb=False):
        ...
        inv_Cl_tot_beam = self.base.inv_Cl_tot_mat[:,:,:]*np.asarray(self.beam)[:,None,:]*np.asarray(self.beam)[None,:,:]
        ...
        def _compute_binned_sum(bin_index):
            ...
            for l1 in range(self.l_bins_squeeze[bin1],self.l_bins_squeeze[bin1+1]):
                for l2 in range(self.l_bins_squeeze[bin2],self.l_bins_squeeze[bin2+1]):
                    ...
                    pref_l12_t2 = (2*l1+1.)*(2*l2+1.)*(-1.)**(l1+l2)/(4.*np.pi)*(Cinv_l_empirical[:,:,None,None,l2]*inv_Cl_tot_beam[None,None,:,:,l1]+Cinv_l_empirical[None,None,:,:,l1]*inv_Cl_tot_beam[:,:,None,None,l2])
                    pref_l12_t0 = (2*l1+1.)*(2*l2+1.)*(-1.)**(l1+l2)/(4.*np.pi)*inv_Cl_tot_beam[:,:,None,None,l1]*inv_Cl_tot_beam[None,None,:,:,l2]
        ...
        if verb:
            ...
            output = list(p.imap(_compute_binned_sum,np.arange(len(bin_list))))
        ...
        if verb:
            ...
            t02_num = list(p.imap(_assemble_output, np.arange(n_elem)))
    ...
    def Tl_numerator_ideal(self, data, verb=False, include_disconnected_term=True, N_cpus=4):
        ...
        # Transform to harmonic space and normalize data by b/C_th
                                    ...
                                    for u_p in self.fields:
                                        ...
                                        for bin1_p in range(self.Nl_squeeze):
                                            for bin2_p in range(self.Nl_squeeze):
                                                ...
                                                if min([bin1_p,bin2_p])>=self.Nl: continue
                                                ...
                                                for bin3_p in range(self.Nl_squeeze):
                                                    ...
                                                    for bin4_p in range(self.Nl_squeeze):
                                                        ...
                                                        if min([bin3_p,bin4_p])>=self.Nl: continue
                                                                    ...
                                                                    for L in range(self.L_bins[binL],self.L_bins[binL+1]):
                                                                        ...
                                                                        for l3 in range(self.l_bins_squeeze[bin3],self.l_bins_squeeze[bin3+1]):
                                                                            for l4 in range(self.l_bins_squeeze[bin4],self.l_bins_squeeze[bin4+1]):
                                                                                ...
                                                                                Cinv_bin = lambda i,j,l: self.base.inv_Cl_tot_mat[[u1,u2,u3,u4][i],[u1_p,u2_p,u3_p,u4_p][j]][l]*([bin1,bin2,bin3,bin4][i]==[bin1_p,bin2_p,bin3_p,bin4_p][j])
