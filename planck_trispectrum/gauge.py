...
k_arr = np.linspace(0.1,501,1001)/r_star
x_arr = np.linspace(0.005,3.00,500).reshape(-1,1)*r_star
s_arr = np.linspace(0.1,501,501)/r_star
...
if transfer=='SW':
    ...
    Tl_k_arr = np.vstack([[np.zeros_like(k_arr) for _ in range(2)],Tl_k_func(k_arr)])
else:
    raise Exception("Wrong transfer function!")

# Compute integrals over Tl(k)
k = np.diff(k_arr).mean()
k2_sq = k_arr**2./(2.*np.pi**2.) # adding k from dlog(k) = dk/k!
xx_arr = x_arr.ravel()
jls = np.asarray([spherical_jn(l,k_arr*x_arr) for l in range(lmax+3)])
...
for l in range(lmax+1):
    pref = Tl_k_arr[l]*k2_sq*dk/k_arr**3.

    # Compute integral h^{ll}_0
    Tl_ints.append(np.sum(Tl_k_arr[l]*k2_sq*dk*jls[l],axis=1))
    
    # Compute integral h^{lL}_{-3}
    tmp_arr = []
    for L in range(abs(l-2),l+3,1):
        tmp_arr.append(np.sum(pref*jls[L],axis=1))
